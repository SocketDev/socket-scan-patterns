 ...
 *   stale path reference (which `lock-step-refs-resolve.mts` catches) — this gate
...
function main(): void {
  ...
  if (values.json) {
    ...
    process.stderr.write(
      `check-lock-step-headers-match: ${diffs.length} quadruplet diff(s) across ${canonicalCount} canonical header(s)`,
    )
